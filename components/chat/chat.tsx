...
export function isStaff(user: any) {
   return user?.profile?.phone_number === '01000000000' ||  user?.profile?.phone_number == '01020507491' || user?.profile?.phone_number == '01041067638' ? true : false;
}
...
export function ScrollParent({ children, atBottom }: PropsWithChildren<any>) {
   ...
   const intoBottom = (smooth: boolean = true) => {
   ...
   const scroll = () => {
      if (messageBox?.current && buttonRef?.current) {
         if (Math.abs(messageBox.current?.scrollHeight - messageBox.current?.scrollTop) - messageBox.current?.clientHeight < 65) {
      ...
      <>
         <ul className="h-full overflow-y-auto" ref={messageBox}>
            {children.length > displayMsg ?
               <li className="border cursor-pointer hover:bg-gray-100 mt-2 mx-auto py-1 rounded-full text-center text-sm w-1/2"
...
const Chat = ({
   ...
   const lid = lectureId ? lectureId[0].lecture : 0;
