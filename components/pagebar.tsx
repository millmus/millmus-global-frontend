...
const Pagebar = ({ totalItems, itemsPerPage, currentPage, url }: IProps) => {
  const showPageArray = 5;
  const maxPage = Math.ceil(totalItems / itemsPerPage); // 마지막 페이지
  const quo = Math.floor(currentPage / showPageArray); // 몫
  const rem = currentPage % showPageArray; // 나머지
  const pageArray = [...Array(showPageArray)]
    .map((_, i) => showPageArray * (rem === 0 ? quo - 1 : quo) + i + 1) // 나머지가 0일 경우에는 quo-1 처리
    .filter((i) => i <= maxPage);
  ...
  const nextPage = () => {
    setPageList((prev) =>
      prev.map((i) => i + showPageArray).filter((i) => i <= maxPage)
    );
  };

  const prevPage = () => {
    const tmp = pageList[0] - showPageArray;
    setPageList([...Array(showPageArray)].map((_, index) => index + tmp));
  };
