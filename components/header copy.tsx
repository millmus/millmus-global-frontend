  ...
  return (
    <header className='sticky top-0 left-0 z-[9999] h-auto w-screen bg-[#14161a] shadow-md'>
      ...
      <div className='mx-auto max-w-[1180px] md:max-w-[330px]'>
        {/* 헤더 상단 */}
        <div className='flex h-[6.25rem] items-center justify-between md:h-14 p-6 md:p-0'>
          {/* 좌측 메뉴 */}
          <nav className='flex items-center overflow-x-hidden text-lg font-medium md:text-sm'>
            {/* 로고 */}
            <Link href='/'>
              <a>
                <svg
                ...
                <a
                  className={cls(
                    navListIndex == 1
                      ? 'text-[#00e7ff]'
                      : '',
                    'flex relative mr-10 md:mr-3 md:whitespace-nowrap'
                  )}
                >
                  <span className='flex flex-col items-center'>
                    <svg className={cls(
                      navListIndex == 1
                        ...
                        : 'fill-white',
                      'mb-[-5px] sm:scale-90'
                    )} width="9" height="8" viewBox="0 0 9 8" xmlns="http://www.w3.org/2000/svg">
                      <path d="M1 6L0 0.5L2.75 3L4.5 0L6.25 3L9 0.5L8 6H1ZM8 7.5C8 7.8 7.8 8 7.5 8H1.5C1.2 8 1 7.8 1 7.5V7H8V7.5Z" />
                    </svg>프</span>
                  <span className='flex flex-col items-center'>
                    <svg className='invisible mb-[-5px] sm:scale-90' width="9" height="8" viewBox="0 0 9 8" xmlns="http://www.w3.org/2000/svg">
                      <path d="M1 6L0 0.5L2.75 3L4.5 0L6.25 3L9 0.5L8 6H1ZM8 7.5C8 7.8 7.8 8 7.5 8H1.5C1.2 8 1 7.8 1 7.5V7H8V7.5Z" />
                    </svg>리미엄클래스</span>
                </a>
              </Link>
          ...
          <Toast count={data?.profile?.unread_notification_count} />

          {/* 우측 메뉴 */}
          <nav className='flex items-center space-x-4 text-sm font-medium md:hidden'>
            {data?.token ? (
              <>
        ...
        <div className={
          cls(
            navList[navListIndex].length ?
              "" : "hidden",
            'flex h-[3.75rem] items-center shadow-[inset_0_1px_0_0_rgba(255,255,255,0.16)] md:h-12 md:shadow-none p-6 md:p-0'
          )}>
          <nav className='flex space-x-10 font-medium md:space-x-[1.25rem] md:overflow-x-scroll md:whitespace-nowrap'>
            {navList[navListIndex] && navList[navListIndex].map((nav) => {
              return (
                <Link key={nav.id} href={nav.url}>
                  <a
                    className={cls(
                      router.asPath ===
                        `${nav.baseUrl}${router.query.page ? `/${router.query.page}` : ''
                        }`
                        ? 'text-[#00e7ff]'
                        : '',
                      'md:text-sm'
