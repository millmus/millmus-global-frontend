...
const BannerSlider = ({ itemList, showArrowInside=true }: Props) => {
  ...
  const dynamicSlidesToShow = containerWidth < 1024 ? 2 : 4;
  ...
  const NextArrow = ({ onClick }: ArrowProps) => (
    <div
      ...
      className={`absolute ${dynamicSlidesToShow === 2 ? 'right-[12px]' : showArrowInside ? 'right-[12px]' : '-right-[0px]'} z-[1] flex aspect-square w-7 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-white`}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="h-5 w-5"
